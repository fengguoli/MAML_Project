...
def tf_read_data(file_name_queue, label_col_idx, record_defaults):
    ...
    # tf.decode_csv：将一行 CSV 数据解析为多个列。record_defaults 参数定义了每列的默认值和数据类型。如果某列为空，将使用默认值填充。
...
def tf_input_pipeline(file_names, batch_size, num_epochs=1, label_col_idx=0, record_defaults=record_defaults):
    ...
    min_after_dequeue = 5000  # 如果 min_after_dequeue 设置为 5000，那么在队列中至少有 5000 个样本时，才会进行随机采样。
    capacity = min_after_dequeue + 3*batch_size  # capacity表示队列的总容量
    # 在深度学习中，数据读取通常是一个相对耗时的操作
    # 通过预取样本，可以在模型训练当前批次数据的同时，提前加载下一批数据，从而减少等待时间，提高整体训练效率。
...
def tfrecord_input_pipeline_test_zb(file_names, num_csv_col, batch_size, num_epochs=1):
    file_name_queue = tf.train.string_input_producer(file_names, num_epochs=num_epochs, shuffle=False)
    reader = tf.TFRecordReader()
    _, serialized_example = reader.read(file_name_queue)
    feature_set = {'label': tf.FixedLenFeature([1], tf.int64), \
                   'data': tf.FixedLenFeature([num_csv_col-1], tf.int64),
                   'hist': tf.FixedLenFeature([20],tf.int64)}
    parsed_feature = tf.parse_single_example(serialized_example, features=feature_set)
    parsed_label = parsed_feature['label']
    parsed_data = parsed_feature['data']
    parsed_hist = parsed_feature['hist']

    min_after_dequeue = 5000
    capacity = min_after_dequeue + 3*batch_size
    feature_batch, label_batch ,hist_batch= tf.train.batch([parsed_data, parsed_label, parsed_hist], \
            batch_size=batch_size, capacity=capacity)
    return feature_batch, label_batch, hist_batch
